...
onready var angle_min: float = angle_min_default if !is_rotation_endless else 0
onready var angle_max: float = angle_max_default if !is_rotation_endless else 360
...
func _process(delta: float) -> void:
	if Engine.editor_hint:
		angle_min = angle_min_default if !is_rotation_endless else 0
		angle_max = angle_max_default if !is_rotation_endless else 360
		rotation_direction = 1 if is_clockwise else -1
